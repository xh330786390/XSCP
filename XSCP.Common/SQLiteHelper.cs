...
namespace XSCP.Common
{
    public class SQLiteHelper
    {
        ...
        {
            string sql = "select distinct(host_key) Domain  from cookies";

            using (SQLiteConnection conn = CreateConnection(CookiePath))
            {
        ...
        {
            string sql = string.Format("select host_key DomainName,name CookieName,value CookieValue,path CookiePath from cookies  where host_key in ({0})", domain);

            using (SQLiteConnection conn = CreateConnection(CookiePath))
            {
                return conn.Query<CookieModel>(sql).ToList();
            }
        }
        #endregion
    }
