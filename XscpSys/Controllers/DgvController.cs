...
namespace XscpSys.Controllers
{
    ...
    public class DgvController
    {
        ...
        public static List<string> Lt_Digit = new List<string>() { "万位", "千位", "百位", "十位", "个位" };
        ...
        public static void SetDgvValue(DataGridView dgv, List<string> ls, int col)
        {
            for (int i = 0; i < ls.Count; i++)
            {
                dgv[col, i].Value = ls[i];
            }
        }
        ...
        public static void RefreshDgvTencenyColorCompare(DataGridView dgv, int startIndex, int endIndex)
        {
            int value = 0;
            for (int i = 0; i < dgv.Rows.Count; i++)
            {
                for (int j = 3; j < dgv.Columns.Count - 1; j++)
                {
                    if (dgv[j, i].Value != null && !string.IsNullOrEmpty(dgv[j, i].Value.ToString()))
                    {
                        try
                        {
                            ...
                            if (value == 1 && j >= startIndex && j <= endIndex) DgvController.SetDgvBackColorStyle(dgv, i, j, Color.YellowGreen, 12);
                        }
                        catch
                        {
                        }
                ...
                if (dgv[j, row].Value != null && !string.IsNullOrEmpty(dgv[j, row].Value.ToString()))
                {
                    try
                    {
                        ...
                        if (j == 10 && (row == 2 || row == 3))
                        {
                            dgv.Rows[row].Cells[j].Style.Font = new Font(DataGridView.DefaultFont, FontStyle.Bold);
                            dgv.Rows[row].Cells[j].Style.Font = new Font("Tahoma", 14);
                            if (row == 2) { dgv.Rows[row].Cells[j].Style.BackColor = Color.Tomato;}
                            else if (row == 3) dgv.Rows[row].Cells[j].Style.BackColor = Color.Orange;    
                        }
